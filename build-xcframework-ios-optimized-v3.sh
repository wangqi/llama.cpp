#!/bin/bash
#
# Optimized llama.cpp XCFramework build script for iOS performance v3
# This version fixes the ARM feature detection issues
#
# Options
...
# Optimized compiler flags for iOS (removed debug -g flag for performance)
...
# ARM64-specific optimization flags for iOS devices  
# Note: We use basic optimizations that don't require specific CPU features
ARM64_DEVICE_FLAGS="-O3 -DNDEBUG -fno-finite-math-only -fvectorize -funroll-loops -flto"

# macOS-specific flags that work with universal binaries
MACOS_OPTIMIZATION_FLAGS="-O3 -DNDEBUG -fno-finite-math-only -fvectorize -funroll-loops -flto"
...
<dict>
    ...
    <string>3.0-iOS-Performance</string>
    <key>BuildDate</key>
...
echo "Building for iOS devices with optimizations..."
# For iOS devices, we'll use the standard build without forcing unsupported features
...
echo ""
echo "=== Build Complete ==="
echo "Optimization version: 3.0-iOS-Performance"
echo "Build date: $(date)"
echo ""
echo "This build includes:"
echo "- Maximum compiler optimizations (-O3, LTO)"
echo "- Vectorization and loop unrolling"
echo "- Metal GPU acceleration with BF16"
echo "- Accelerate framework BLAS"
echo ""
echo "Note: Advanced ARM features (DOTPROD, I8MM) are available at runtime"
echo "      based on device capabilities, not forced at compile time."
echo ""
echo "Done"